...
bool CFamiTrackerDoc::ImportInstruments(CFamiTrackerDoc *pImported, int *pInstTable)
{
	...
	for (unsigned int s = 0; s < MAX_SEQUENCES; ++s) {
		for (int t = 0; t < SEQ_COUNT; ++t) {
			...
			// // // 5B
			if (pImported->GetSequenceItemCountS5B(s, t) > 0) {
				CSequence *pImportSeq = pImported->GetSequenceS5B(s, t);
				int index = GetFreeSequenceS5B(t);
				if (index != -1) {
					CSequence *pSeq = GetSequenceS5B(unsigned(index), t);
					pSeq->Copy(pImportSeq);
					// Save a reference to this sequence
					SequenceTableS5B[s][t] = index;
				}
			}
		}
	}
			...
			case INST_S5B:		// // //
				{
					...
					for(unsigned int i = 0; i < CInstrumentS5B::SEQUENCE_COUNT; i++) {
						int freeSeq = this->GetFreeSequenceS5B(i);
						int oldSeq = pInstrument->GetSeqIndex(i);
						if (freeSeq != -1) {
							if( pInstrument->GetSeqEnable(i) ) {
								this->GetSequenceS5B(unsigned(freeSeq), i)->Copy( this->GetSequenceS5B(unsigned(oldSeq), i) );
							}
							pInstrument->SetSeqIndex(i, freeSeq);
						}
					}
...
void CFamiTrackerDoc::SelectExpansionChip(unsigned char Chip, bool Move)
{
	...
	int OldChip = m_iExpansionChip, Current = OldChip;
	int CH = m_iRegisteredChannels;
	if (Move) {
		if ((OldChip & SNDCHIP_VRC6) && !(Chip & SNDCHIP_VRC6)) {
			for (int j = GetChannelPosition(CHANID_MMC5_SQUARE1, Current); j < CH; j++)
				for (unsigned int i = 0; i < m_iTrackCount; ++i)
					m_pTracks[i]->CopyChannel(j - 3, j);
			Current &= ~SNDCHIP_VRC6;
			CH -= 3;
		}
		...
		if ((OldChip & SNDCHIP_N163) && !(Chip & SNDCHIP_N163)) {
			for (int j = GetChannelPosition(CHANID_FDS, Current); j < CH; j++)
				for (unsigned int i = 0; i < m_iTrackCount; ++i)
					m_pTracks[i]->CopyChannel(j - m_iNamcoChannels, j);
			Current &= ~SNDCHIP_N163;
			CH -= m_iNamcoChannels;
		}
		if ((OldChip & SNDCHIP_FDS) && !(Chip & SNDCHIP_FDS)) {
			for (int j = GetChannelPosition(CHANID_VRC7_CH1, Current); j < CH; j++)
				for (unsigned int i = 0; i < m_iTrackCount; ++i)
					m_pTracks[i]->CopyChannel(j - 1, j);
			Current &= ~SNDCHIP_FDS;
			CH -= 1;
		}
		if ((OldChip & SNDCHIP_VRC7) && !(Chip & SNDCHIP_VRC7)) {
			for (int j = GetChannelPosition(CHANID_S5B_CH1, Current); j < CH; j++)
				for (unsigned int i = 0; i < m_iTrackCount; ++i)
					m_pTracks[i]->CopyChannel(j - 6, j);
			Current &= ~SNDCHIP_VRC7;
			CH -= 6;
		}
	}
	// // // Remove frames and patterns of unused channels
	for (unsigned int i = 0; i < m_iTrackCount; ++i)
		for (int j = GetChannelPosition(CHANNELS, Current); j < MAX_CHANNELS; j++) {
			for (int k = 0; k < MAX_PATTERN; k++) {
				m_pTracks[i]->SetEffectColumnCount(j, 0);
				m_pTracks[i]->ClearPattern(j, k);
			}
			for (int k = 0; k < MAX_FRAMES; k++)
				m_pTracks[i]->SetFramePattern(k, j, 0);
		}
	...
	/*Current = OldChip;*/
	if (Move) {
		if ((Chip & SNDCHIP_VRC7) && !(OldChip & SNDCHIP_VRC7)) {
			CH += 6;
			for (int j = GetChannelCount() - 1; j >= GetChannelPosition(CHANID_S5B_CH1, Current) + 6; j--)
				for (unsigned int i = 0; i < m_iTrackCount; ++i) {
					m_pTracks[i]->CopyChannel(j, j - 6);
				}
			Current |= SNDCHIP_VRC7;
		}
		if ((Chip & SNDCHIP_FDS) && !(OldChip & SNDCHIP_FDS)) {
			CH += 1;
			for (int j = GetChannelCount() - 1; j >= GetChannelPosition(CHANID_VRC7_CH1, Current) + 1; j--)
				for (unsigned int i = 0; i < m_iTrackCount; ++i) {
					m_pTracks[i]->CopyChannel(j, j - 1);
				}
			Current |= SNDCHIP_FDS;
		}
		if ((Chip & SNDCHIP_N163) && !(OldChip & SNDCHIP_N163)) {
			CH += m_iNamcoChannels;
			for (int j = GetChannelCount() - 1; j >= GetChannelPosition(CHANID_FDS, Current) + static_cast<signed>(m_iNamcoChannels); j--)
				for (unsigned int i = 0; i < m_iTrackCount; ++i) {
					m_pTracks[i]->CopyChannel(j, j - m_iNamcoChannels);
				}
			Current |= SNDCHIP_N163;
		}
		...
		if ((Chip & SNDCHIP_VRC6) && !(OldChip & SNDCHIP_VRC6)) {
			CH += 3;
			for (int j = GetChannelCount() - 1; j >= GetChannelPosition(CHANID_MMC5_SQUARE1, Current) + 3; j--)
				for (unsigned int i = 0; i < m_iTrackCount; ++i) {
					m_pTracks[i]->CopyChannel(j, j - 3);
				}
			Current |= SNDCHIP_VRC6;
		}
	}
...
void CFamiTrackerDoc::SetNamcoChannels(int Channels, bool Move)
{
	...
	if (!ExpansionEnabled(SNDCHIP_N163))
		SelectExpansionChip(m_iExpansionChip | SNDCHIP_N163, true);
	int OldCount = m_iNamcoChannels;
	if (Channels < OldCount && Move) for (unsigned int i = 0; i < m_iTrackCount; ++i) {
		for (int j = GetChannelPosition(CHANID_FDS, m_iExpansionChip); j < GetChannelCount(); j++)
			m_pTracks[i]->CopyChannel(j - (OldCount - Channels), j);
	}
	ASSERT(Channels <= 8);
	m_iNamcoChannels = Channels;
	// // //
	if (Channels > OldCount && Move) for (unsigned int i = 0; i < m_iTrackCount; ++i) {
		for (int j = GetChannelCount() + (Channels - OldCount) - 1; j >= GetChannelPosition(CHANID_FDS, m_iExpansionChip); j--)
			m_pTracks[i]->CopyChannel(j, j - (Channels - OldCount));
	}
}
...
void CFamiTrackerDoc::RemoveUnusedInstruments()
{
	...
	for (unsigned int i = 0; i < MAX_SEQUENCES; ++i) {
		for (int j = 0; j < SEQ_COUNT; ++j) {
			...
			if (GetSequenceS5B(i, j)->GetItemCount() > 0) {
				bool Used = false;
				for (int k = 0; k < MAX_INSTRUMENTS; ++k) {
			...
			for (unsigned int j = 0; j < m_iTrackCount; ++j) {
				for (unsigned int Frame = 0; Frame < m_pTracks[j]->GetFrameCount(); ++Frame) {
					...
					for (unsigned int Row = 0; Row < m_pTracks[j]->GetPatternLength(); ++Row) {
						...
						if (pNote->Note == NONE || pNote->Note > B || Index == MAX_INSTRUMENTS) continue;
...
void CFamiTrackerDoc::MakeKraid()			// // // Easter Egg
{
	...
	for (int i = 0; i < 12; i += 6) {
		kraidRow->Note = C; kraidRow->Octave = 2; SetDataAtPattern(0, 1, 2, i     , kraidRow);
		                    kraidRow->Octave = 3; SetDataAtPattern(0, 1, 2, i +  2, kraidRow);
		kraidRow->Note = D; kraidRow->Octave = 2; SetDataAtPattern(0, 1, 2, i + 12, kraidRow);
		                    kraidRow->Octave = 3; SetDataAtPattern(0, 1, 2, i + 14, kraidRow);}
	for (int i = 0; i < 6; i += 2) {
		kraidRow->Note = E;  kraidRow->Octave = 2 + i / 2; SetDataAtPattern(0, 2, 2, i     , kraidRow);
		kraidRow->Note = Fs;                               SetDataAtPattern(0, 2, 2, i +  6, kraidRow);
		kraidRow->Note = F;                                SetDataAtPattern(0, 2, 2, i + 12, kraidRow);
		kraidRow->Note = B;  kraidRow->Octave = 1 + i / 2; SetDataAtPattern(0, 2, 2, i + 18, kraidRow);}
	kraidRow->Note = E; kraidRow->Octave = 1;	SetDataAtPattern(0, 4, 2,  0, kraidRow);
	kraidRow->Note = C; kraidRow->Octave = 2;	SetDataAtPattern(0, 3, 2, 12, kraidRow);
	kraidRow->Note = E;							SetDataAtPattern(0, 3, 2,  0, kraidRow);
	kraidRow->Note = G;							SetDataAtPattern(0, 3, 2, 16, kraidRow);
	kraidRow->Note = A;							SetDataAtPattern(0, 3, 2, 18, kraidRow);
	kraidRow->Note = B;							SetDataAtPattern(0, 3, 2,  4, kraidRow);
												SetDataAtPattern(0, 3, 2, 10, kraidRow);
												SetDataAtPattern(0, 3, 2, 22, kraidRow);
	kraidRow->Note = C; kraidRow->Octave = 3;	SetDataAtPattern(0, 3, 2,  6, kraidRow);

	// Pulse 2
	kraidRow->Instrument = 0;
	kraidRow->Note = As; kraidRow->Octave = 2;	SetDataAtPattern(0, 0, 1, 22, kraidRow);
	kraidRow->Note = B;							SetDataAtPattern(0, 0, 1,  4, kraidRow);
	...
	kraidRow->Note = Fs;						SetDataAtPattern(0, 0, 1, 12, kraidRow);
	...
	kraidRow->Note = G; 						SetDataAtPattern(0, 1, 1,  0, kraidRow);
	...
	kraidRow->Note = A; 						SetDataAtPattern(0, 0, 1,  6, kraidRow);
	...
	kraidRow->Note = Ds; kraidRow->Octave = 3; kraidRow->Instrument = 1;
